...
def friends_of_friends(graph, user):
    ...
    for x in friends(graph, user):
        friends_of_friends_set = friends_of_friends_set.union(set(friends(graph, x)))
    friends_of_friends_set = friends_of_friends_set.difference(set(graph.neighbors(user)))
...
assert friends_of_friends(rj, 'Romeo') == set(['Paris', 'Escalus', 'Tybalt', 'Nurse', 'Capulet'])
assert friends_of_friends(rj, 'Juliet') == set(['Mercutio', 'Paris', 'Escalus', 'Montague', 'Benvolio'])


def common_friends(graph, user1, user2):
    """Returns the set of friends that user1 and user2 have in common."""
    return set(graph.neighbors(user1)).intersection(set(graph.neighbors(user2)))
...
def number_of_common_friends_map(graph, user):
    ...
    nodes_set.remove(user)
    nodes_set = nodes_set.difference(set(graph.neighbors(user)))
    nodes_set_list = []
    nodes_set_list.extend(nodes_set)
    map_dictionary = {}
    for x in range(len(nodes_set_list)):
        if len(common_friends(graph, user, nodes_set_list[x])) == 0:
            x += 1
        else:
            map_dictionary[nodes_set_list[x]] = len(common_friends(graph, user, nodes_set_list[x]))    
